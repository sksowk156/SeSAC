...
class IOManage private constructor() {
    companion object {
        ...
        fun getIOManage(): IOManage = instance ?: IOManage().also { instance = it }
    ...
    suspend fun askFirstQuestion() {
        ...
        while (!nextLine.isNullOrEmpty() && !nextLine.equals("exit", true)) {
            when (nextLine.toInt()) {
                1 -> {
                    this.printList(eventManage.getEventList())
                ...
                2 -> {
                    this.printList(companyManage.getCompanyList())
                ...
                3 -> {
                    this.printList(idolgroupManage.getIdolgroupList())
        ...
        while (!nextLine.isNullOrEmpty() && !nextLine.equals("exit", true)) {
            when (nextLine.toInt()) {
                1 -> {
                    ...
                    nextLine = consoleScanner()
                    val eventList = eventManage.getEventListByGroupName(nextLine) // 리스트 정보
                    if (eventList.isNotEmpty()) {
                        this.printList(eventList)
                    } else {
            ...
            if (event != null) {
                ...
                when (nextLine2.toInt()) {
                    1 -> {
                        ...
                        val idolgroup = idolgroupManage.getIdolgroupList().find { it.name == nextLine2 }
                        if (idolgroup != null) {
        ...
        while (!nextLine.isNullOrEmpty() && !nextLine.equals("exit", true)) {
            when (nextLine.toInt()) {
                1 -> {
                    ...
                    nextLine = consoleScanner()
                    val company = companyManage.getCompanyListByGroupName(nextLine)
                    if (company != null) {
                        println(company)
                    } else {
    ...
    private fun getIdolgroup(): Idolgroup {
        println("그룹명을 입력해주세요")
    ...
    private fun getIdolList(): MutableList<Idol> {
